...
function get_row_volume(df_rebinned::DataFrame; thickness = unique(df_rebinned.E1)[2] - unique(df_rebinned.E1)[1])
    totalArea = sum(get_area.(df_rebinned.minE, df_rebinned.maxE, df_rebinned.a, df_rebinned.b))
    volume = totalArea*thickness
    return volume
end

function get_total_volume(df_rebinned::DataFrame; thickness = unique(df_rebinned.E1)[2] - unique(df_rebinned.E1)[1])
    volume = 0.0
    for e in unique(unique(df_rebinned.E1))
        volume += get_row_volume(df_rebinned[df_rebinned.E1 .== e, :])
    end
    return volume
end